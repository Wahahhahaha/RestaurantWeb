  ...
  <div class="report-card">
    ...
    <div style="padding: 0 32px 32px;">
      <table class="report-table">
        ...
        <tbody>
          <?php if (empty($tanggalSemua)): ?>
            <tr>
              <td colspan="3">There's no transaction for today</td>
            </tr>
          <?php else: ?>
            <?php foreach ($tanggalSemua as $i => $tgl): ?>
              <tr>
                <td><?= $loop->iteration ?? ($i + 1) ?></td>
                <td><?= \Carbon\Carbon::parse($tgl)->locale('id')->translatedFormat('d M Y') ?></td>
                <td class="action-cell">
                   <form action="/dailyreport" method="get" target="_blank">
                  ...
                  <form action="/pdfdailyreport?tanggal=<?= urlencode($tgl) ?>" method="get" target="_blank" >
                    <input type="hidden" name="tanggal" value="<?= $tgl ?>">
                    <button type="submit" class="btn-action btn-pdf">
                    <i class="bi bi-file-earmark-pdf">PDF</i>
                  </button>
                  </form>
                  <form action="/exceldailyreport?tanggal=<?= urlencode($tgl) ?>" method="get" target="_blank">
                    <input type="hidden" name="tanggal" value="<?= $tgl ?>">
                    <button type="submit" class="btn-action btn-excel">
                    <i class ="bi bi-file-earmark-spreadsheet">Excel</i>
                  </button>
                  </form>
                </td>
              </tr>
